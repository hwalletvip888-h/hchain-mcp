...
export function registerTradeTools(server: McpServer, auth: Auth | null = null): void {
  ...
  server.tool(
    ...
    async ({ chainIndex }) => {
      if (!auth) return AUTH_REQUIRED("READ");
      try {
        const data = await onchainosApi.getTokens(auth, chainIndex);
        return toResult(data);
      } catch (e) { return toError(e); }
    },
  );
  ...
  server.tool(
    ...
    "CAT:[链上-Swap] | ## 功能：获取最优 Swap 报价，返回输出金额、价格影响、路由明细\n## 场景：用于交易前比价、获取最优路由、估计交易成本。不构造交易，不广播\n## 关键词：报价, quote, swap, 路由, 价格影响, slippage\n## 参数：\n##   - chainIndex: 链索引\n##   - fromTokenAddress: 卖出代币合约地址\n##   - toTokenAddress: 买入代币合约地址\n##   - amount: 卖出数量（人类可读数，非最小单位）\n##   - slippagePercent: 滑点百分比，默认 0.5\n## 鉴权：⚠️ 需要 API Key（只读）\n## 风险：READ — 只读查询\n## 返回量：微小 ~2KB\n## 关联：onchainos_get_tokens 确认代币可交易 → 本工具获取报价 → onchainos_build_approve_transaction 授权",
    {
      ...
      amount: z.string().describe("卖出数量（人类可读格式，如 '1.5' 表示 1.5 个代币）"),
      slippagePercent: z.string().optional().describe("滑点百分比。默认 '0.5'（0.5%），如 '1' 表示 1%"),
    },
    { readOnlyHint: true },
    async ({ chainIndex, fromTokenAddress, toTokenAddress, amount, slippagePercent }) => {
      if (!auth) return AUTH_REQUIRED("READ");
      try {
        const data = await onchainosApi.getQuote(auth, {
          chainIndex, fromTokenAddress, toTokenAddress, amount,
          slippagePercent: slippagePercent ?? "0.5",
        });
        return toResult(data, {
          nextSteps: [
            { action: "构建授权", tool: "onchainos_build_approve_transaction", condition: "若 allowance 不足" },
            { action: "构建交易", tool: "onchainos_build_swap", condition: "授权完成后" },
          ],
        });
      } catch (e) { return toError(e); }
    },
  ...
  server.tool(
    ...
    async ({ chainIndex, tokenContractAddress, approveAmount }) => {
      ...
      try {
        const data = await onchainosApi.approveTransaction(auth, { chainIndex, tokenContractAddress, approveAmount });
        return toResult(data, {
          nextSteps: [
            { action: "广播交易", tool: "onchainos_broadcast_transaction" },
            { action: "构建Swap", tool: "onchainos_build_swap" },
          ],
        });
      } catch (e) { return toError(e); }
    },
  ...
  server.tool(
    ...
    async ({ chainIndex, fromTokenAddress, toTokenAddress, amount, userWalletAddress, slippagePercent, approveTransaction }) => {
      ...
      try {
        ...
        return toResult(data, {
          nextSteps: [
            { action: "模拟交易", tool: "onchainos_simulate_transaction" },
            { action: "广播交易", tool: "onchainos_broadcast_transaction" },
          ],
        });
      } catch (e) { return toError(e); }
