...
export function registerWalletTools(server: McpServer, auth: Auth | null): void {
  ...
  server.tool(
    ...
    "CAT:[链上-账户] | ## 功能：查询指定地址在多链上的代币余额总览\n## 场景：用于查看持仓、检查余额是否足够交易、多链资产查询\n## 关键词：余额, balance, 持仓, portfolio, 多链\n## 参数：\n##   - address: 钱包地址\n##   - chains: 链索引，逗号分隔（如 \"1,56,137\"），默认查所有支持的链\n## 鉴权：⚠️ 需要 API Key（只读）\n## 风险：READ — 只读查询\n## 返回量：中等 ~10KB\n## 关联：onchainos_get_token_info 查代币 → 本工具查余额 → onchainos_get_transaction_history 查历史",
    {
    ...
    async ({ address, chains }) => {
      ...
      try {
        const data = await onchainosPrivateApi.getTotalValue(auth, address, chains ?? "1,56,137,8453,501");
        return toResult(data, {
          nextSteps: [
            { action: "查看交易历史", tool: "onchainos_get_transaction_history", params: { address, chains } },
            { action: "获取Gas配置", tool: "onchainos_get_gas_config", params: {} },
          ],
        });
  ...
  server.tool(
    ...
    async ({ address, chains, limit }) => {
      if (!auth) return AUTH_REQUIRED("READ");
      try {
        const data = await onchainosPrivateApi.getTransactionsByAddress(auth, address, chains ?? "1,56,137,8453,501", limit);
        return toResult(data);
      } catch (e) { return toError(e); }
    },
  );
  ...
  server.tool(
    ...
    async ({ txHash, chainIndex }) => {
      if (!auth) return AUTH_REQUIRED("READ");
      try {
        const data = await onchainosPrivateApi.getTransactionDetail(auth, txHash, chainIndex);
        return toResult(data);
      } catch (e) { return toError(e); }
    },
  );
